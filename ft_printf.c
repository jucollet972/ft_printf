...
int ft_get_format_precision(char **str)
{
    ...
    if (**str == '.')
    {
        ...
        if (*(*str) && *(*str) >= '0' && *(*str) <= '9')
        {
            ...
            index = ft_count_integer(precision);
            while (index-- && **str)
                (*str)++;
            return (precision);
        }
...
char *ft_char(va_list arg, t_format *format)
{
    ...
    if (c == 0)
    {
        format->type = format->type | CHARNULL;
         return (ft_strdup("\0"));
    }
    if (!(str = ft_strnew(1)))
        return (NULL);
...
char  *ft_integer_accurate_str(char *type_str, t_format *format)
{
    ...
    if (format->type & INTEGER && format->flags & ZERO)
...
char  *ft_accurate_string(char *type_str, t_format *format)
{
    ...
    {
        if (format->precision > (len = (ft_strlen(type_str))))
        {
...
char *ft_format_string(va_list arg, t_format *format)
{
  ...
  //  printf("%s\n", type_str);
    ...
    if (len < format->width)
    {
        if (format->flags & SPACE && (format->flags & ZERO))
            len += 1;
        len = format->width - len;
    }
    else
	   len = 0;
    (format->flags & CHARNULL) ? format->width -= 1 : 0;
    (format->flags & MORE && *ref_str != '-') ? len -= 1 : 0;
    (format->flags & HASH && format->type & OCTAL) ? len -= 1 : 0;
...
int ft_constuct_str(char *ref, va_list arg)
{
    ...
    {
        if (!(temp = ft_strjoin(temp, ft_strsub(ref, index, end_str - 1))))
            return (-1);
        index += end_str;
        ...
        else if (!(temp = ft_strjoin(temp, format_string)))
            return (-1);
    }
    ...
    else if (!(temp = ft_strjoin(temp, ft_strsub(ref, index, ft_strlen(ref + index)))))
        return (-1);
    len = ft_strlen(temp);
    write(1, temp, len);
    return (len);
